...
import type { Service, PricingOption } from '@/data/services';
...
const ServiceModal = ({ service, isOpen, onClose }: ServiceModalProps) => {
  ...
  const handleContractClick = (pricingOption: PricingOption) => {
    if (pricingOption.price === 'gratuita') {
      // Handle free services differently - maybe redirect to contact
      window.open(`https://wa.me/34639877578?text=Hola, me interesa la ${pricingOption.type} de ${service?.title}`, '_blank');
      return;
    }
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            {service.pricing.map((option, index) => (
              <Card key={index} className="border-2 hover:border-primary/50 transition-colors">
                ...
                <CardContent className="space-y-6">
                  ...
                  >
                    {option.price === 'gratuita' ? 'Solicitar' : 'Comprar'} {option.type}
                  </Button>
                </CardContent>
              </Card>
            ))}
          ...
          <div className="mt-6 p-4 bg-muted/50 rounded-lg text-center">
            <p className="text-sm text-muted-foreground">
              ...
              <Button 
                variant="link" 
                className="p-0 ml-1 h-auto text-primary underline"
                onClick={() => window.open('https://wa.me/34639877578?text=Hola, tengo dudas sobre qué plan elegir para mi proyecto', '_blank')}
              >
